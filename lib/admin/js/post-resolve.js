...
jQuery(function ($) {
  ...
  function resolveInput($input, key) {
    // Request-generation guard: a stale response must never overwrite a newer value's hint.
    var mySeq = requestSeq[key] = (requestSeq[key] || 0) + 1;
    ...
    fetch(nmPostResolve.endpoint + separator + 'ids=' + ids.join(','), {
      ...
      .then(function (results) {
        if (mySeq !== requestSeq[key]) { return; }
