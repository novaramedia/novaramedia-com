 ...
 * renders the primary-zone badges, and wires click-to-field. Post data
 * comes from the nm-post-resolve client (lib/admin/js/post-resolve.js).
 * Display strings share the contract of title_hint_html() (PHP) and
 * renderHint() (lib/meta/js/cmb2-post-search-field-hints.js) — keep the
 * three in sync.
...
jQuery(function ($) {
  ...
  function paint() {
    ...
    zones.forEach(function (entry) {
      ...
      if (!cache[entry.id]) {
        // Resolve hasn't reported on this id (fetch failed, or the
        // nm-post-resolve client is missing): don't blank a server-rendered
        // zone. Unchanged ids
  ...
  function resolveAndPaint() {
    ...
    if (!missing.length || typeof nmPostResolveClient === 'undefined') {
      $preview.find('[data-nm-banner]').prop('hidden', true);
      paint();
      return;
    }

    var isCurrent = nmPostResolveClient.track('atf-preview');

    nmPostResolveClient.resolve(missing)
