...
if ( $is_product_linked || $more_on_section ) {
  if ( $is_product_linked ) {
    $product_term_object = get_the_sub_category( $featured_post_id, true );

    if ( empty( $product_term_object ) ) {
      return;
    }

    $product_category_link = get_term_link( $product_term_object );
    $media_type = get_the_top_level_category( $featured_post_id );
    ...
    $product_category_link = get_term_link( $more_on_section );
    $label = 'More ' . $more_on_section->name;
  }
  ?>
<div class="mt-2 font-size-8 font-weight-bold only-desktop">
  <a href="<?php echo $product_category_link; ?>" class="ui-hover"><span class="ui-dot ui-dot--red"></span><?php echo $label; ?></a>
</div>
  <?php
}
?>
